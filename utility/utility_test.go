...
func TestShouldReturnCorrectFileHash(t *testing.T) {
	checksumSomeFile := CollectiveSHA256Hash([]string{"some_file.pem"})
	checksumTestSomeFile := CollectiveSHA256Hash([]string{"test/some_file.pem"})
...
func TestShouldReturnEmptyFileHashWhenNoPathsPassed(t *testing.T) {
	checksum := CollectiveSHA256Hash([]string{})
