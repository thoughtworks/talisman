...
func TestShouldNotFlagSafeText(t *testing.T) {
	...
	NewFileContentDetector().Test(ChecksumCompare{calculator: nil, ignoreConfig: talismanrc.NewTalismanRC(nil)}, additions, &talismanrc.TalismanRC{}, results)
...
func TestShouldIgnoreFileIfNeeded(t *testing.T) {
	...
	NewFileContentDetector().Test(ChecksumCompare{calculator: nil, ignoreConfig: talismanrc.NewTalismanRC(nil)}, additions, talismanrc.NewTalismanRC([]byte(talismanRCContents)), results)
...
func TestShouldNotFlag4CharSafeText(t *testing.T) {
	...
	NewFileContentDetector().Test(ChecksumCompare{calculator: nil, ignoreConfig: talismanrc.NewTalismanRC(nil)}, additions, talismanRC, results)
	assert.False(t, results.HasFailures(), "Expected file to not to contain base64 encoded texts")
}
