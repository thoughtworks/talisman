...
func TestShouldIgnorePasswordPatterns(t *testing.T) {
	...
	NewPatternDetector(customPatterns).Test([]gitrepo.Addition{}, additions, ignores, results)
...
func DetectionOfSecretPattern(filename string, content []byte) (*DetectionResults, []gitrepo.Addition, string) {
	results := NewDetectionResults()
	additions := []gitrepo.Addition{gitrepo.NewAddition(filename, content)}
	NewPatternDetector(customPatterns).Test([]gitrepo.Addition{}, additions, talismanRC, results)
